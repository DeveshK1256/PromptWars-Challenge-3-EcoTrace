/**
 * @module firebase
 * @description EcoTrace Firebase data service layer. Re-exports the
 * authentication methods from {@link module:firebase-auth} and adds
...
export const ecoService = {
  /* ── Auth methods (spread from firebase-auth) ────────────────────── */
  ...authService,
  ...
  async saveFootprint(user, result) {
    ...
    if (!user || user.isDemo) {
      ...
      addActivity(
        `Saved a footprint score of ${saved.totalKg.toLocaleString()} kg CO₂/year.`,
        "footprint",
      );
      return saved;
    }
    const runtime = await initFirebase();
    const { db, dbMod } = runtime;
    const footprintRef = await dbMod.addDoc(
      dbMod.collection(db, "users", user.uid, "footprints"),
      { ...record, createdAt: dbMod.serverTimestamp() },
    );
    await dbMod.setDoc(
