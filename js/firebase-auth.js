 ...
 * @description EcoTrace Firebase authentication service layer. Handles
 * Firebase SDK initialisation (app, auth, firestore, App Check), user
 * serialisation, sign-in / sign-up / sign-out flows, password reset,
 * and account deletion — with seamless local-storage fallback ("demo mode")
...
import {
  ...
  STORAGE_KEYS,
  DEFAULT_PROFILE, DEFAULT_DISPLAY_NAME,
} from "./demo-store.js";
...
/** @type {Promise<Object|null>|undefined} Singleton init promise. */
let firebasePromise;

/** @type {Object|null|undefined} Cached Firebase runtime (app, auth, db, etc.). */
let firebaseRuntime;
    ...
    firebasePromise = Promise.all([
      ...
      try {
        ...
        if (recaptchaKey && !recaptchaKey.startsWith('__')) {
          ...
          logInfo('firebase-auth', 'App Check initialised');
        }
      } catch (appCheckError) {
        logWarn('firebase-auth', 'App Check unavailable', appCheckError);
      }
 ...
 */
export async function deleteQuerySnapshot(dbMod, querySnapshot) {
...
/**
 * All authentication-related methods, exported as a single object
 * so that `firebase.js` can spread them into `ecoService`.
 * @type {Object}
 */
export const authService = {
  ...
  async signInWithEmail(email, password) {
    ...
    if (!runtime) {
      ...
      if (!account) {
        throw createAuthError(
          "auth/user-not-found",
          "No account exists for this email. Use Create account first.",
        );
      }
      ...
      const profile = saveDemoProfile(
        account.profile || createDemoAccountProfile(account.email, account.displayName),
      );
      addActivity("Signed in with demo email credentials.", "auth");
      return profile;
    }
    const credential = await runtime.authMod.signInWithEmailAndPassword(
      runtime.auth, email, password,
    );
    const user = serializeFirebaseUser(credential.user);
    await ensureUserDocument(user);
    return user;
  ...
  async createEmailAccount(email, password, displayName = DEFAULT_DISPLAY_NAME) {
    ...
    const credential = await runtime.authMod.createUserWithEmailAndPassword(
      runtime.auth, email, password,
    );
  ...
  async sendPasswordReset(email) {
    ...
    try {
      ...
      logError('firebase-auth', "Password reset error:", err);
      throw err;
    }
  },
  ...
  async deleteAccount(user) {
    ...
    const { db, dbMod } = runtime;
    const footprints = await dbMod.getDocs(
      dbMod.collection(db, "users", user.uid, "footprints"),
    );
    const activities = await dbMod.getDocs(
    ...
    if (runtime.auth.currentUser) {
      await runtime.authMod.deleteUser(runtime.auth.currentUser);
    }
  },
};
